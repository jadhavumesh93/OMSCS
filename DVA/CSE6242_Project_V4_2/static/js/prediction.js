...
function createCharts(btnClick, divId, chartDivId, index, data, url){
    ...
    dataset = "../static/stockjson.json"
    var tooltip = d3.tip().attr("id", `tooltip_${index}`)
    svg.call(tooltip)
    if(divId == "leftpane"){
        //dataset = "../static/bivariate_GBM.csv"
        //dataset = "../static/bivariate_GBM.csv"
        dataset = "../static/sp500_with_features.csv"
        getChartJson(btnClick, div_height, div_width, margin, svg, data, tooltip, width, height, index, url, divId);
        //getChartSPX(btnClick, div_height, div_width, margin, svg, dataset, tooltip, width, height, index, divId);
    }
    else if(divId == "rightpane"){
        //dataset = "../static/stockjson.json"d.
        dataset = "../static/prediction.csv"
        getChartJson(btnClick, div_height, div_width, margin, svg, data, tooltip, width, height, index, url, divId);
        //getChartJson(btnClick, div_height, div_width, margin, svg, dataset, tod.RNNoltip, width, height, index);
    }
    console.log(d3)
    //var tooltip = d3.tip().attr("id", "tooltip")
}

function getChartSPX(reload, div_height, div_width, margin, svg, dataset, tooltip, width, height, index, divId){
    ...
    d3.csv(dataset, function(d){
        if(divId == "leftpane")
        return {date : new Date(d.DATE), SP500 : +d.SP500}
        else
        return {date : new Date(d.DATE), Actual : +d.Actual, GBM : +d.GBM, RNN : +d.RNN, RF : +d.RF, Ensemble : +d.Ensemble, MAE : +d.MAE}
    }).then(function(data){
        ...
        var normalized_data = getNormalizedData(data, json_keys)
        ...
        if(divId == "leftpane"){
            minY = d3.min(normalized_data, function(d){ return d.SP500 });
            maxY = d3.max(normalized_data, function(d){ return d.SP500 });
        }
        else{
            minY = minVal
            maxY = maxVal
...
function getChartJson(reload, div_height, div_width, margin, svg, dataset, tooltip, width, height, index, url, divId){
    console.log("reload = ", reload)
    ...
    .then(function(data){
        ...
        */
        ...
        var yAxisLabel_a = y_axis.append("text").attr("id", `y_axis_label_${index}`).attr("transform", "rotate(-90)")
                  ...
                  .attr("x",(-(height - margin.top - margin.bottom )/2))
                  .attr("dy", "1em")
                  .style("text-anchor", "end").text(function(d) {if(divId=="rightpane") return "Index price"; else return "Actual vs Prediction"})
                  .style("font-size", 14).attr("fill", "black")
        ...
        json_keys.forEach(js => {
            ...
            if(js != "DATE"){
                ...
                .attr("fill", "none")
                .attr("stroke", function(d, j) { return colorScheme(i) })
                .attr("stroke-width", function(d) { if(js == "Ensemble") return 3; else return 1.5})
                .attr('stroke-dasharray', function(d, i) { console.log("djs = ", js); if(js == "MAE") return 5,5; else return 0,0})
                .attr("d", d3.line()
                .x(function(d) { return x(d.DATE) })
                .y(function(d) { return y(d[js]) })
                )
                ...
                // Create legend
                legend_circle = circle_g.selectAll('circle_'+js).data(normalizedData).enter().append('circle').attr('id', `circle_${index}_`+js)
                                .attr('r', 5).attr('cx', div_width - margin.right).attr('cy', margin.top + (i + 1)*20)
                                .attr('fill', (d, j) => colorScheme(i))
                       
                legend_text = circle_g.selectAll('legend_'+js).data(normalizedData).enter().append('text').attr('id', `legend_${index}`+js)
                                        .attr('x', div_width - margin.right + 10).attr('y', margin.top + 3 + (i + 1)*20)
                                        .text((d, j) => js).attr('fill', (d, j) => colorScheme(i)).style('font-size', 12)
            }
            i += 1
        ...
        json_keys.forEach(js => {
            if(js.toUpperCase() != "DATE"){
                svg.select(`#${js}_${index}`).on('mousemove', function(event){
                    ...
                    if(index == "b"){
                    d3.select(`#tooltip_${index}`).style('top', `${yPos + div_height + 150}px`).style('left', `${xPos+div_width}px`).style('background-color', "lightblue").html(`Date : ${formatTime(d.DATE)}<br>Price : ${formatDecimal(d[js])}<br>Sym: ${js}`).style('opacity', 0.9).style('padding', '8px')
                    }
                    else{
                        d3.select(`#tooltip_${index}`).style('top', `${yPos + div_height + 150}px`).style('left', `${xPos}px`).style('background-color', "lightblue").html(`Date : ${formatTime(d.DATE)}<br>Price : ${formatDecimal(d[js])}<br>Sym: ${js}`).style('opacity', 0.9).style('padding', '8px')
                    }
                })
            }
        })
